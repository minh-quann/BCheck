<template>
  <div class="upload-wrapper">
    <div class="upload-panel card shadow p-5 mb-5">
      <h2 class="card-title mb-4 text-primary text-center">Bộ chứng từ thanh toán</h2>
      ...
      <div class="card image-section p-4 mb-4 shadow-sm" v-for="(bct, index) in documents" :key="index">
        <div class="d-flex justify-content-between align-items-center mb-3">
          <h5 class="mb-0">Bộ chứng từ {{ index + 1 }}</h5>
          <button class="btn btn-sm btn-danger" @click="removeDocument(index)" v-if="documents.length > 1">
            Xóa BCT
          </button>
        </div>
        ...
        <div class="mb-3">
          ...
          <input type="file" accept="image/*" class="form-control" @change="handleFile($event, index, 'meal')" multiple />
        </div>

        <div class="mb-3">
          <label class="form-label">Ảnh biên lai</label>
          <input type="file" accept="image/*" class="form-control" @change="handleFile($event, index, 'hotel')" multiple />
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      endpoint: 'https://nhomvungtrom2.app.n8n.cloud/webhook-test/upload',
    };
  },
  computed: {
    canSubmit() {
